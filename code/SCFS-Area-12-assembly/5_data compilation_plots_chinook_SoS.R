...
# Name: Emma Atkinson
# Description: Chinook salmon stock-recruit data compilation.
# Notes: Data compilation (Chinook) adapted from Peacock et al. 2013
...
dir.plot <- "C:/Users/Emma Atkinson/Desktop/Research/SoS/Analysis/Plots/Chinook"

setwd(dir.gen)

### Prepping data for use with Steph's code ###
nuseds <- read.csv("updated_nuseds_2017.csv")
data <- filter(nuseds, SPECIES=="Chinook")

# --- Chinook --- #
chinook <- data %>% 
  select(ANALYSIS_YR, AREA, WATERBODY, MAX_ESTIMATE, GFE_ID) 

chinook <- chinook[,c(1,2,3,4,5)]
names(chinook) <- c("Yr","Area","River", "Spawners","GFE_ID")

### Filtering chinook data ###
...
chinook$River <- factor(chinook$River, levels=levels(nuseds$WATERBODY)) # Change levels of river column to reflect those of nuseds dataframe

for (i in 1:length(unique(chinook$River))){
  chinook$streamID[chinook$River==unique(chinook$River)[i]] <- unique(nuseds$GFE_ID[nuseds$WATERBODY==unique(chinook$River)[i]])
  chinook$popID[chinook$River==unique(chinook$River)[i]] <- i
}

# How many years does that data currently span? # 
range(chinook$Yr) # Actually start in 1929, but will stick with 1954 for consistency

# Because data are very sparse before 1954, we won't include 1950-54 in total # of years
# Alternative - include all years #
ny.spawn <- length(unique(chinook$Yr[chinook$Yr>=1954&chinook$Yr<=2017])) 
...
Z.lengths<-c(); Z.max.spawn <- c(); L<-unique(chinook$popID);
for(i in 1:length(L)){
  z1<-subset(chinook, popID==L[i]);
...
for(i in 1:length(R.overall)){
  z1<-subset(chinook, popID==R.overall[i])
  R.Area<-c(R.Area,z1$Area[1])
  R.River<-c(R.River,z1$popID[1])
}

# Which rivers met criteria? #
chinook.rivers <- unique(chinook$River[chinook$popID%in%R.River])
as.character(chinook.rivers)
runIDs <- as.character(chinook.rivers)
...
for (run in runIDs){
  years <- unique(chinook[chinook$River==run,]$Yr)
  for (i in years){
    l <- length(chinook[chinook$River==run & chinook$Yr==i,]$Yr)
    if (l > 1){
      r <- which(chinook$River==run & chinook$Yr==i)
      chinook[r[1],]$Spawners = sum(chinook[r,]$Spawners, na.rm=T)
      chinook = chinook[-c(r[2:length(r)]),]
    }
...
plot.prod = function(run){
  data = chinook[chinook$River==run,]	
...
for(i in as.character(runIDs)) {
  #windows(9,3.5)
  #print(i)
  pdf(paste(i,'_chinook_Feb-2019.pdf',sep=''),5,5)
  plot.prod(i)
  dev.off()
}
...
for (pop in unique(dat$Population)){
  status.table[status.table$Population==pop,]$S_avg = chinook_Savg(pop)[3]
  status.table[status.table$Population==pop,]$S_avg_year = chinook_Savg(pop)[1]
  status.table[status.table$Population==pop,]$historic_avg = chinook_Savg(pop)[2]
}
...
setwd(dir.plot)
write.csv(status.table, file = "1_chinook_status_table_Feb-2019.csv", row.names=F)
